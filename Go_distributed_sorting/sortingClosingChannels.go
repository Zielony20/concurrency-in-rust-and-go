...
//		fmt.Println(fmt.Sprintf("%d	nasłuchuje %d", processID,values))
		time.Sleep(1 * time.Second)

		select {
//		case lin, _ := <-rightChanOut:			
//			*values = append(*values, lin)
//          _, mx = update(values)  //nie updatuje tutaj
//			send(rightChanIn, mx)
//			removeElement(values, mx)
//			_, mx = update(values)
		case end = <-terminationChOut:
		default:
//			fmt.Println(fmt.Sprintf("%d	nie ma nic do roboty", processID))		
			continue
		}
	}
